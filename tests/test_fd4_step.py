"""FD order=4: multi-step comparison against Fortran.

Runs with fd_order=4, fd_order_bound=2 (Boussinesq benchmark).
Verifies all non-pressure fields match Fortran to machine precision
over 10 steps. Pressure (p0 l=0) has known larger errors from
the first-order backward difference at the ICB.
"""
...
_REF_DIR = os.path.join(
    ...
    "..", "samples", "dynamo_benchmark_fd4", "fortran_ref",
)
...
_FD4_ENV = {
    ...
    "MAGIC_RADRATIO": "0.35",
    "MAGIC_FD_ORDER": "4", "MAGIC_FD_ORDER_BOUND": "2",
...
def test_fd4_field(results, attr, tol, step):
    """FD order=4 field at each step must match Fortran."""
