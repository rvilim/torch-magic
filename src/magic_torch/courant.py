...
def _courant_mag_damp(vrc, vtc, vpc, brc, btc, bpc, cf2, af2, valri2, valhi2,
                      or4_3=_or4_3, or2_3=_or2_3, orho1_3=_orho1_3,
                      orho2_3=_orho2_3, Ost2=_Ost2):
    """CFL with magnetic field + Alfven damping (l_cour_alf_damp=True)."""
    vflr2 = orho2_3 * vrc * vrc
    vflh2 = (vtc * vtc + vpc * vpc) * Ost2 * orho2_3

    valr = brc * brc * LFfac * orho1_3
    valr2 = valr * valr / (valr + valri2)

    valh2 = (btc * btc + bpc * bpc) * LFfac * Ost2 * orho1_3
    valh2m = valh2 * valh2 / (valh2 + valhi2)

    vr2max = (or4_3 * (cf2 * vflr2 + af2 * valr2)).amax(dim=(1, 2))
    vh2max = (or2_3 * (cf2 * vflh2 + af2 * valh2m)).amax(dim=(1, 2))
...
def _courant_mag_nodamp(vrc, vtc, vpc, brc, btc, bpc, cf2, af2,
    ...
    vr2max = (or4_3 * (cf2 * vflr2 + af2 * valr)).amax(dim=(1, 2))
    vh2max = (or2_3 * (cf2 * vflh2 + af2 * valh2)).amax(dim=(1, 2))
...
def _courant_nomag(vrc, vtc, vpc, cf2,
                   or4_3=_or4_3, or2_3=_or2_3, orho2_3=_orho2_3, Ost2=_Ost2):
    """CFL without magnetic field (XSH_COURANT=0)."""
    vflr2 = orho2_3 * vrc * vrc
    vflh2 = (vtc * vtc + vpc * vpc) * Ost2 * orho2_3

    vr2max = (cf2 * or4_3 * vflr2).amax(dim=(1, 2))
    vh2max = (cf2 * or2_3 * vflh2).amax(dim=(1, 2))
...
def courant_check(vrc, vtc, vpc, brc=None, btc=None, bpc=None,
                  ...
                  or4_3=_or4_3, or2_3=_or2_3, orho1_3=_orho1_3,
                  orho2_3=_orho2_3, Ost2=_Ost2,
                  delxr2_in=delxr2, delxh2_in=delxh2):
    ...
    Args:
        ...
        or4_3, or2_3, orho1_3, orho2_3: radial profiles (n_r, 1, 1)
        Ost2: theta profile (1, n_theta, 1)
        delxr2_in, delxh2_in: grid spacings (n_r,)

    Returns:
        (dtrkc, dthkc): per-level Courant time steps, shape (n_r,)
    """
    cf2 = courfac * courfac
    ...
    if use_mag:
        ...
        if l_cour_alf_damp:
            valri2 = ((0.5 * (1.0 + opm)) ** 2 / delxr2_in).reshape(-1, 1, 1)
            valhi2 = ((0.5 * (1.0 + opm)) ** 2 / delxh2_in).reshape(-1, 1, 1)
        ...
        vr2max, vh2max = _courant_nomag(vrc, vtc, vpc, cf2,
                                         or4_3, or2_3, orho2_3, Ost2)

    # dtrkc/dthkc per level: sqrt(delx2/v2max) where v2max > 0, else large
    large = torch.tensor(1e10, dtype=DTYPE, device=DEVICE)
    dtrkc = torch.where(vr2max > 0, torch.sqrt(delxr2_in / vr2max), large)
    dthkc = torch.where(vh2max > 0, torch.sqrt(delxh2_in / vh2max), large)

    return dtrkc, dthkc


def dt_courant(dt, dtMax, dtrkc_min, dthkc_min):
    ...
    Args:
        ...
        dtrkc_min: minimum radial Courant time step (float)
        dthkc_min: minimum horizontal Courant time step (float)
